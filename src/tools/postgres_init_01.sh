...
$AUTH psql $CONNECTION ${DB} -c "SET ROLE semehr_admin; CREATE TABLE semehr.semehr_results(SOPInstanceUID varchar(64), semehr_results JSONB, PRIMARY KEY (SOPInstanceUID));"

# Create the table holding a count of each CUI
$AUTH psql $CONNECTION ${DB} -c "SET ROLE semehr_admin; CREATE TABLE semehr.cui_count (cui varchar(20) primary key, count bigint);"

# Create the table holding CUI for doc
$AUTH psql $CONNECTION ${DB} -c "SET ROLE semehr_admin; CREATE TABLE semehr.cui_sop (cui varchar(20), SOPInstanceUID varchar(64));"
...
$AUTH psql $CONNECTION ${DB} -c "CREATE INDEX patientid ON semehr.semehr_results ((semehr_results->>'PatientID'));"
$AUTH psql $CONNECTION ${DB} -c "CREATE INDEX cui_as_array ON semehr.semehr_results USING GIN (semehr.annotation_array_as_text_array(semehr_results, 'cui'));"
$AUTH psql $CONNECTION ${DB} -c "CREATE INDEX sty_as_array ON semehr.semehr_results USING GIN (semehr.annotation_array_as_text_array(semehr_results, 'sty'));"
$AUTH psql $CONNECTION ${DB} -c "CREATE INDEX pref_as_text ON semehr.semehr_results USING GIN (semehr.annotation_array_as_text(semehr_results, 'pref') gin_trgm_ops);"
$AUTH psql $CONNECTION ${DB} -c "CREATE INDEX str_as_text ON semehr.semehr_results USING GIN (semehr.annotation_array_as_text(semehr_results, 'str') gin_trgm_ops);"
$AUTH psql $CONNECTION ${DB} -c "CREATE INDEX str_as_english ON semehr.semehr_results USING GIN (to_tsvector('english', semehr.annotation_array_as_text(semehr_results, 'str')));"
$AUTH psql $CONNECTION ${DB} -c "CREATE INDEX pref_as_english ON semehr.semehr_results USING GIN (to_tsvector('english', semehr.annotation_array_as_text(semehr_results, 'pref')));"
$AUTH psql $CONNECTION ${DB} -c "CREATE INDEX content_date ON semehr.semehr_results (semehr.cast_to_date(semehr_results->>'ContentDate'));"
$AUTH psql $CONNECTION ${DB} -c "CREATE INDEX modalities ON semehr.semehr_results (regexp_split_to_array(semehr_results->>'ModalitiesInStudy', '\\\\'));"
